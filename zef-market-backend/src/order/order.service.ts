...
import { Order, OrderItem } from './entities/order.schema';
...
return this.createOrderDependOnPaymentMethod(userId,createOrderDto.paymentMethodType)
  ...
  async findOne(id: string, user: JwtPayloadType) {
    ...
    if (
      ...
      throw new UnauthorizedException(
        'you are no allowed to access this route',
      );
    }
  ...
  async toggleOrderToPaidStatue(id: string) {
  ...
  async toggleOrderToDeliverdStatue(id: string) {
    const order = await this.orderModel.findById(id);
  ...
  async createOrderAfterPaypal(paymentResult: any) {
    // جلب كارت المستخدم
    const cart = await this.cartService.getCurrentUserCart(
      paymentResult.payer.payer_id,
    );
