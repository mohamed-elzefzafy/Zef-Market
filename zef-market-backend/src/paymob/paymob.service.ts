...
//     // ترتيب الحقول حسب وثائق Paymob
//     const hmacString = [
//       query.amount_cents || '',
//       query.created_at || '',
//       query.currency || '',
//       query.error_occured === 'true' ? 'true' : 'false',
//       query.has_parent_transaction === 'true' ? 'true' : 'false',
//       query.id || '',
//       query.integration_id || '',
//       query.is_3d_secure === 'true' ? 'true' : 'false',
//       query.is_auth === 'true' ? 'true' : 'false',
//       query.is_capture === 'true' ? 'true' : 'false',
//       query.is_refunded === 'true' ? 'true' : 'false',
//       query.is_standalone_payment === 'true' ? 'true' : 'false',
//       query.is_voided === 'true' ? 'true' : 'false',
//       query.order || '',
//       query.owner || '',
//       query.pending === 'true' ? 'true' : 'false',
//       query['source_data.pan'] || query.source_data_pan || '',
//       query['source_data.sub_type'] || query.source_data_sub_type || '',
//       query['source_data.type'] || query.source_data_type || '',
//       query.success === 'true' ? 'true' : 'false',
//     ]
//       .map(String)
//       .join('');
...
//     const isValid = hmacReceived === hmacCalculated;
//     if (!isValid) {
...
//       const userId = obj.payment_key_claims?.user_id;
...
//         );
//       } else {
//         console.log('❌ Payment failed or missing data:', JSON.stringify(obj, null, 2));
//       }
...
export class PaymobService {
  ...
  constructor(
    @Inject(forwardRef(() => OrderService))
    private readonly orderService: OrderService,
    private readonly userService: UsersService, // إضافة UserService
  ) {}
  ...
  async verifyHmac(query: any): Promise<boolean> {
    ...
    // ترتيب الحقول حسب وثائق Paymob مع decode لـ created_at
    const hmacString = [
      query.amount_cents || '',
      decodeURIComponent(query.created_at || ''), // Decode created_at
      query.currency || '',
      query.error_occured === 'true' ? 'true' : 'false',
      query.has_parent_transaction === 'true' ? 'true' : 'false',
      query.id || '',
  ...
  private async getPaymentKey(
    ...
    // جلب Paymob user_id من MongoDB user
    const user = await this.userService.findOne(userId);
    if (!user) {
      throw new HttpException('User not found', HttpStatus.NOT_FOUND);
    }
    const paymobUserId =  user._id.toString(); // استخدام paymobUserId أو _id كبديل
    ...
    const { data } = await axios.post(
      `${this.baseUrl}/acceptance/payment_keys`,
      {
        ...
        extras: {
          userId: paymobUserId, // تمرير paymobUserId
        },
      },
    );
    return data.token;
  ...
  async handleWebhook(payload: any) {
    try {
      ...
      const obj = payload?.obj;
      if (!obj || payload.type !== 'TRANSACTION') { // تجاهل TOKEN webhooks
        console.log('❌ Ignoring non-TRANSACTION webhook or invalid payload');
        return { received: false };
      }
      ...
      if (success && userId && merchantOrderId) {
        console.log('✅ Payment success, updating order...');
        const orderResult = await this.orderService.createOrderDependOnPaymentMethod(
          userId.toString(), // تحويل userId لـ string
          'paymob',
          merchantOrderId,
        );
